...
@functools.lru_cache
def get_sequences_saved(num_sequences=1000, num_workers=None, filename=None):

    if filename is not None:
        try:
            results = load_results_from_json(filename)
            logger.info(f"{filename} loaded from cache.")
            return results
        except FileNotFoundError:
            logger.info(f"{filename} not found, generating new sequences.")
